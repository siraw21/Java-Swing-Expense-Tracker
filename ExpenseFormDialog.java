...
public class ExpenseFormDialog extends JDialog {
    ...
    // True if the user clicked Save successfully
    ...
    public ExpenseFormDialog(JFrame parent, Expense existing, ExpenseDAO dao) {
        ...
        if (existing != null) fillFields(); // pre-fill if editing
    }
    ...
    private void handleSave() {
        ...
        if (amtText.isEmpty()) { warn("Amount is required."); return; }
        double amount;
        try {
            amount = Double.parseDouble(amtText);
            if (amount <= 0) { warn("Amount must be greater than zero."); return; }
        } catch (NumberFormatException e) {
            warn("Amount must be a number (e.g. 12.50).");
            return;
        }

        // 2. Validate date (must be YYYY-MM-DD)
    ...
    /** Dashboard calls this to know whether it should refresh the table */
